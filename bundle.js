...
function GetImages() {
	return new Promise(function (resolve, reject) {
		fetch('/images').then(function (resp) {
			return resp.json();
		}).then(function (data) {
			resolve(data);
		}).catch(function (err) {
...
function SaveProgress(progress) {
	_axios2.default['post']('/save', { data: progress });
}

//load already saved canvas element
function LoadProgress() {
	return new Promise(function (resolve, reject) {
		_axios2.default['post']('/load').then(function (data) {
			if (data.status === 200) {
				resolve(data.data);
			} else {
				reject(data.data);
...
},{"./logic.js":32,"axios":1,"jquery":27}],30:[function(require,module,exports){
'use strict';
...
(0, _jquery2.default)(document).ready(function () {
  ...
  (0, _jquery2.default)('#submit').click(function () {
    var val = (0, _jquery2.default)('input.form-control[type=file]');
    (0, _api.UploadImages)(val).then(function (data) {
      initImagesList();
    });
  });

  (0, _jquery2.default)('div.canvas div.block').on({ drop: _logic.DropImage, dragover: _logic.DragOver });
  (0, _jquery2.default)('div.canvas-control img').click(controlClick);
  (0, _jquery2.default)('#addText').click(_logic.AddTextElement);
});

//load previously saved elements, and initialize drag handle
function loadSavedElement() {
  (0, _api.LoadProgress)().then(function (data) {
    (0, _jquery2.default)("div.canvas div.block").append(data);
    (0, _logic.initFunctionDrag)();
  });
}
...
function initImagesList() {
  (0, _api.GetImages)().then(function (data) {
    ...
    _jquery2.default.each(data, function (i, img) {
      parentImgContainer.append('<li>\n        \t\t\t\t\t\t<img src=' + img + ' class="image-list" draggable="true"/>\n        \t\t\t\t\t\t</li>');
    });
    (0, _jquery2.default)('.image-list').on('dragstart', _logic.DragImage);
  });
}
...
},{"./api.js":29,"./logic.js":32,"jquery":27}],32:[function(require,module,exports){
'use strict';
...
exports.initFunctionDrag = initFunctionDrag;
...
function DragElement(ev) {
  ...
  var currentLeft = parseInt(style.getPropertyValue("left"), 10) - ev.clientX;
  var currentTop = parseInt(style.getPropertyValue("top"), 10) - ev.clientY;
...
function TextDoneEdit(ev) {
  var spanElement = (0, _jquery2.default)('#' + ev.target.parentElement.id);
...
function TextDoubleClick(ev) {
  var content = (0, _jquery2.default)('#' + ev.target.id)[0].textContent;
  (0, _jquery2.default)('#' + ev.target.id).empty();
  (0, _jquery2.default)('#' + ev.target.id).append('<input  type="text" value=\'' + content + '\'></input>');
  (0, _jquery2.default)('#' + ev.target.id).find('input').on({ blur: TextDoneEdit });
}

//set any given ID element as active
function setActive(id) {
  (0, _jquery2.default)("div.canvas div.block>.active").removeClass("active");
  (0, _jquery2.default)('#' + id).addClass('active');
}

//initialize drag event for newly loaded elements
function initFunctionDrag() {
  var content = (0, _jquery2.default)("div.canvas div.block");
...
function DropImage(ev) {
  ev.preventDefault();
  var next_id = Math.round(Math.random() * 100);
  var data = ev.originalEvent.dataTransfer.getData("image");
  if (data) {
    //if image is a new image taken from the image list
    var element_id = (0, _helper.GetNextId)('img_obj');
    (0, _jquery2.default)("div.canvas div.block").append('<img id=\'' + element_id + '\' draggable=\'true\' src=\'' + data + '\'/>');
    (0, _jquery2.default)('#' + element_id).on({ dragstart: DragElement, click: ClickElement });
    setActive(element_id);
  } else {
    //if image is already on canvas, just needed to change position
    var _element_id = ev.originalEvent.dataTransfer.getData("id");
    var offset = ev.originalEvent.dataTransfer.getData("pos").split(',');
    var dm = document.getElementById(_element_id);
...
function AddTextElement(ev) {
  var element_id = (0, _helper.GetNextId)('text_obj');
  (0, _jquery2.default)("div.canvas div.block").append('<span id=\'' + element_id + '\' draggable=\'true\'>' + (0, _helper.GetDefaultText)() + '</span>');
  (0, _jquery2.default)('#' + element_id).on({ dragstart: DragElement, click: ClickElement, dblclick: TextDoubleClick });
  setActive(element_id);
}

//event triggered when delete button is clicked, remove current active element
function RemoveElement() {
  (0, _jquery2.default)("div.canvas div.block").find('.active').remove();
}

//event triggered when save button is clickd, save current canvas elements
function SaveCanvas() {
  (0, _api.SaveProgress)((0, _jquery2.default)("div.canvas div.block")[0].innerHTML);
}

},{"./api.js":29,"./helper.js":30,"jquery":27}]},{},[31]);
